        ...
        <div class="true-false">
          ...
          <form>
            ...
            <div class="center-align lg-mar-top">
              ...
              <p class="text-mute center-align">Made any changes since pressing this button? If so, press it again.</p>
            </div>
          </form>
  ...
  methods: {
    ...
    createLearningObject() {
      ...
      if (! this.jQueryAlreadyAvailable) {
        HTML += `<script data-has-jquery type="text/javascript" src="https://code.jquery.com/jquery-1.12.1.min.js"><\/script>`;
      }

      var uniquid = Math.floor(Date.now() / 1000);

      HTML += `<script type="text/javascript">`;
                HTML += `(function () { var questions = [`;
                for (var i = 0; i < this.questions.length; i++) {
                ...
                HTML += `];`;
                HTML += `var allAnswered=function (){var counter=0; for (var i=0; i < questions.length; i++){if (questions[i].answeredCorrectly !=null){counter++;}}if (counter==questions.length){return true;}return false;}; var getCorrectAnswersPercentage=function (){var counter=0; for (var i=0; i < questions.length; i++){if (questions[i].answeredCorrectly === true){counter++;}}return (counter / questions.length) * 100;};`;
                HTML += `$(document).ready(function (){var questionElements=$('.` + uniquid + `.trueFalse .question'); var answerElements=$('.` + uniquid + `.trueFalse .answer'); var explanationElements=$('.` + uniquid + `.trueFalse .explanation'); var trueButtonElements=$('.` + uniquid + `.trueFalse .button.true'); var falseButtonElements=$('.` + uniquid + `.trueFalse .button.false');`;
                  HTML += `for (var i=0; i < questionElements.length; i++){questionElements.eq(i).attr('data-index', i); answerElements.eq(i).attr('data-index', i); explanationElements.eq(i).attr('data-index', i); trueButtonElements.eq(i).attr('data-index', i); falseButtonElements.eq(i).attr('data-index', i); questionElements.eq(i).html(questions[i].question);}`;
                  HTML += `$('.` + uniquid + `.trueFalse .button').on('click', function (){var btnClass=$(this).attr('class'); var index=$(this).attr('data-index'); trueButtonElements.eq(index).css({'background': 'rgba(26, 188, 156,0.85)', 'box-shadow': 'inset 0px -6px 0px rgba(26, 188, 156,1.0)'}); falseButtonElements.eq(index).css({'background': 'rgba(231, 76, 60,0.85)', 'box-shadow': 'inset 0px -6px 0px rgba(231, 76, 60,1.0)'});`;
                    HTML += `if (btnClass.indexOf('true') >=0){$(this).css({'background': 'rgba(149, 165, 166,0.85)', 'box-shadow': 'inset 0px -6px 0px rgba(149, 165, 166,1.0)'});`;
                      HTML += `if (questions[index].answer===true){/* correct */ answerElements.eq(index).text('You are correct! You selected TRUE!');`;
                        HTML += `answerElements.eq(index).css({'background': 'rgba(46, 204, 113, 0.85)', 'border-bottom': '7px solid rgba(46, 204, 113, 1)', 'display': 'block'});`;
                        HTML += `if (questions[index].hasExplanations){explanationElements.eq(index).text(questions[index].correctExplanation);explanationElements.eq(index).css({'display': 'block', 'border-bottom': '7px solid rgba(46, 204, 113, 1)', 'background': 'rgba(46, 204, 113, 0.85)'});}questions[index].answeredCorrectly=true;}else{/* incorrect */ answerElements.eq(index).text('You are incorrect! You selected TRUE!');`;
                          HTML += `answerElements.eq(index).css({'background': 'rgba(231, 76, 60, 0.85)', 'border-bottom': '7px solid rgba(231, 76, 60, 1)', 'display': 'block'});`;
                          HTML += `if (questions[index].hasExplanations){explanationElements.eq(index).text(questions[index].incorrectExplanation); explanationElements.eq(index).css({'display': 'block', 'border-bottom': '7px solid rgba(243, 156, 18, 1)', 'background': 'rgba(243, 156, 18, 0.85)'});}`;
                          HTML += `questions[index].answeredCorrectly=false;}}`;
                      HTML += `else if (btnClass.indexOf('false') >=0){$(this).css({'background': 'rgba(149, 165, 166,0.85)', 'box-shadow': 'inset 0px -6px 0px rgba(149, 165, 166,1.0)'});`;
                        HTML += `if (questions[index].answer===true){/* incorrect */ answerElements.eq(index).text('You are incorrect! You selected FALSE!');`;
                          HTML += `answerElements.eq(index).css({'background': 'rgba(231, 76, 60, 0.85)', 'border-bottom': '7px solid rgba(231, 76, 60, 1)', 'display': 'block'});`;
                          HTML += `if (questions[index].hasExplanations){explanationElements.eq(index).text(questions[index].incorrectExplanation); explanationElements.eq(index).css({'display': 'block', 'border-bottom': '7px solid rgba(243, 156, 18, 1)', 'background': 'rgba(243, 156, 18, 0.85)'});}questions[index].answeredCorrectly=false;}`;
                        HTML += `else{/* correct */ answerElements.eq(index).text('You are correct! You selected FALSE!'); answerElements.eq(index).css({'background': 'rgba(46, 204, 113, 0.85)', 'border-bottom': '7px solid rgba(46, 204, 113, 1)', 'display': 'block'});`;
                          HTML += `if (questions[index].hasExplanations){explanationElements.eq(index).text(questions[index].correctExplanation); explanationElements.eq(index).css({'display': 'block', 'border-bottom': '7px solid rgba(46, 204, 113, 1)', 'background': 'rgba(46, 204, 113, 0.85)'});}questions[index].answeredCorrectly=true;}}`;
                        HTML += `if (allAnswered()) {$('.` + uniquid + `.trueFalse .score').text('You got '+ getCorrectAnswersPercentage().toFixed(2) + '% of the questions correct!');}else {$('.` + uniquid + `.trueFalse .score').text('waiting for you to complete the quiz..');}});`;
                  HTML += `$('.` + uniquid + `.trueFalse .reset').on('click', function (){for (var i=0; i < answerElements.length; i++){trueButtonElements.eq(i).css({'background': 'rgba(26, 188, 156,0.85)', 'box-shadow': 'inset 0px -6px 0px rgba(26, 188, 156,1.0)'}); falseButtonElements.eq(i).css({'background': 'rgba(231, 76, 60,0.85)', 'box-shadow': 'inset 0px -6px 0px rgba(231, 76, 60,1)'}); answerElements.eq(i).text(''); answerElements.eq(i).css('display', 'none'); explanationElements.eq(i).text(''); explanationElements.eq(i).css('display', 'none'); questions[i].answeredCorrectly = null;`;
                    HTML += `$('.` + uniquid + `.trueFalse .score').text('waiting for you to complete the quiz..');}});});`;
              HTML += `})();<\/script>`;
              HTML += `<div class="` + uniquid + ` trueFalse" style="width: 100%; max-width: 1200px; margin: 0 auto; background: #ecf0f1;">`;
                HTML += `<div class="container" style="box-sizing: border-box; width: 100%; text-align: center; font-family: Arial,Helvetica,Verdana;">`;
                  HTML += `<div class="row" style="box-sizing: border-box; margin: 0 auto; text-align: center; font-family: Arial,Helvetica,Verdana; padding: 2px; width: 100%; float: left;">`;
                    HTML+= `<div class="col twelve" style="box-sizing: border-box; text-align: center; font-family: Arial,Helvetica,Verdana; min-height: 60px; display: inline-block; float: left; margin-left: 0px; margin-right: 0px; width: 100%;"><h1 style="box-sizing: border-box; text-align: center; font-family: Arial,Helvetica,Verdana;" data-object-title>` + this.title + `</h1></div></div>`;
              for (var i = 0; i < this.questions.length; i++) {
                HTML += `<div class="row" style="box-sizing: border-box; margin: 0 auto; text-align: center; font-family: Arial,Helvetica,Verdana; padding: 2px; width: 100%; float: left;">`;
                  HTML += `<div class="col twelve" style="box-sizing: border-box; text-align: center; font-family: Arial,Helvetica,Verdana; min-height: 60px; display: inline-block; float: left; margin-left: 0px; margin-right: 0px; width: 100%;">`;
                    HTML += `<span class="question" style="box-sizing: border-box; font-family: Arial,Helvetica,Verdana; color: #525151; display: block; margin-top: 30px; border-top: 2px solid #ddd; text-align: center; padding: 24px; line-height: 26px; font-size: 26px; background: #fefefe; border-left: 10px solid #ddd;border-right: 2px solid #ddd;"></span>`;
                    HTML += `<span class="answer" style="box-sizing: border-box; text-align: center; font-family: sans-serif; display: none; padding: 18px; color: #fff; text-shadow: 2px 2px 2px rgba(44, 62, 80,1.0); letter-spacing: 2px; font-size: 22px;"></span>`;
                    HTML += `<span class="explanation" style="box-sizing: border-box; text-align: center; font-family: sans-serif; display: none; padding: 18px; letter-spacing: 2px; font-size: 20px; color: #fff; text-shadow: 2px 2px 2px rgba(44, 62, 80,1.0);"></span> </div></div>`;
                HTML += `<div class="row" style="box-sizing: border-box; margin: 0 auto; text-align: center; font-family: Arial,Helvetica,Verdana; padding: 2px; width: 100%; float: left;">`;
                  HTML += `<div class="col twelve" style="box-sizing: border-box; text-align: center; font-family: Arial,Helvetica,Verdana; min-height: 60px; display: inline-block; float: left; margin-left: 0px; margin-right: 0px; width: 100%;">`;
                    HTML += `<span class="button true float-left" style="box-sizing: border-box; font-family: Arial,Helvetica,Verdana; float: left; display: inline-block; width: 48.5%; padding: 17px; line-height: 24px; text-align: center; font-weight: bolder; background: rgba(26, 188, 156,0.85); color: #fff; cursor: pointer; transition: all 0.20s ease-in-out; box-shadow: inset 0px -6px 0px rgba(26, 188, 156,1); font-size: 24px;">T</span> &nbsp;`;
                    HTML += `<span class="button false float-right" style="box-sizing: border-box; font-family: Arial,Helvetica,Verdana; float: right; display: inline-block; width: 48.5%; padding: 17px; line-height: 24px; text-align: center; font-weight: bolder; background: rgba(231, 76, 60,0.85); color: #fff; cursor: pointer; transition: all 0.20s ease-in-out; box-shadow: inset 0px -6px 0px rgba(231, 76, 60,1.0); font-size: 24px;">F</span></div></div>`;
              }
              HTML+= `<div class="row" style="box-sizing: border-box; margin: 0 auto; text-align: center; font-family: Arial,Helvetica,Verdana; padding: 2px; width: 100%; float: left;"> <div class="col twelve" style="box-sizing: border-box; text-align: center; font-family: Arial,Helvetica,Verdana; min-height: 60px; display: inline-block; float: left; margin-left: 0px; margin-right: 0px; width: 100%;"> <span class="score" style="box-sizing: border-box; font-family: Arial,Helvetica,Verdana; text-align: center; display: inline-block; border: 1px solid #ddd; border-radius: 3px; color: #8a8a8a; padding: 18px; background: #fefefe; font-weight: bolder; font-size: 18px; margin-top: 35px;">start when you are ready..</span> </div></div><div class="row" style="box-sizing: border-box; margin: 0 auto; text-align: center; font-family: Arial,Helvetica,Verdana; padding: 2px; width: 100%; float: left;"> <div class="col twelve" style="box-sizing: border-box; text-align: center; font-family: Arial,Helvetica,Verdana; min-height: 60px; display: inline-block; float: left; margin-left: 0px; margin-right: 0px; width: 100%;"> <span class="button reset" style="box-sizing: border-box; font-family: Arial,Helvetica,Verdana; border: 1px solid rgba(155, 89, 182, 1); display: inline-block; width: 48.5%; padding: 17px; line-height: 24px; font-size: 24px; text-align: center; background: rgba(155, 89, 182, 0.85); color: #fff; cursor: pointer; transition: all 0.20s ease-in-out; box-shadow: inset 0px -6px 0px rgba(155, 89, 182, 1); margin-top: 40px; font-weight: bolder;">RESET</span> </div></div></div></div>`;
    ...
    populateQuestions () {
      ...
      if (questionsData.indexOf('data-has-jquery') > -1) {
        this.jQueryAlreadyAvailable = false;
      }
